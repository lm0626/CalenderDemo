...
public class LunarCalendar {
	...
	// ====== 传回农历 y年的生肖
	static final public String animalsYear(int year) {
	...
	public String getLunarDate(int year_log, int month_log, int day_log, boolean isday) {
		...
		if(!isday){
			...
			for(int i = 0; i < Holiday.solarHoliday.length; i++){
				//返回公历节假日名称
				String sd = Holiday.solarHoliday[i].split(" ")[0];  //节假日的日期
				String sdv = Holiday.solarHoliday[i].split(" ")[1]; //节假日的名称
			...
			for(int i = 0; i < Holiday.lunarHoliday.length; i++){
				//返回农历节假日名称
				String ld = Holiday.lunarHoliday[i].split(" ")[0];   //节假日的日期
				String ldv = Holiday.lunarHoliday[i].split(" ")[1];  //节假日的名称
